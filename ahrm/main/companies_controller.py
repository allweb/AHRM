...
def treedata(request,id):
    ...
    def def_depfather(id,compID):
        objComp = Company.objects.get(id=compID)
        if id == None:
            objDepList = Department.objects.filter(department_father = None,company = objComp)
        else :
            obj = Department.objects.get(id=id)
            objDepList = Department.objects.filter(department_father = obj,company = objComp)
        if len(objDepList) == 0 :
            json_obj = '{"text":' + '"' + obj.name + '",' + '"id":' + '"department@@@' + str(obj.id) + '",' + '"cls":"file","leaf":"true"'                         
            return json_obj
        else:
...
def viewdetail(request):
    ...
    if objType == 'company':
        ...
        if objType == 'department':
            ...
            desc = dept.desc
            id = dept.id    
            btEdit = '<input type="submit" value="Edit Department" name="dep_edit"/>'
            btDelete ='<input type="submit" value="Delete Department" name="dep_delete" onclick="return beforeDelete();"/>'          
    htmlstring = '<form id="form1" name="form1" method="post" action="/department/add/">' 
    htmlstring += '<table border="0"><tr><td width="160"><b>'+ objType +' name</b></td><td width="372">:'+ name +'</td></tr>'
    htmlstring += '<tr><td valign="top"><b>Description</b></td><td>:<label>'+desc+'</label></td></tr>'    
    htmlstring += '<tr><td align="right"><input type="submit" value="Add Department" name="dep_add"/></td><td >'+btEdit+'&nbsp;'+btDelete+'</td></tr>'
    htmlstring += '<tr><td><input type="hidden" value="' + objType + '" name="type" /></td><td ></td></tr>'
    htmlstring += '<tr><td><input type="hidden" value="'+ str(id) +'" name="id"/></td><td ></td></tr></table></form>'
    jsonData = Context({                         
                         'jsondata' : htmlstring,                         
