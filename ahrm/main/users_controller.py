...
def submit_login(request):   
    ...
    if user is not None:
        if user.is_active:
            ...
            t = loader.get_template('login_template.html')
            c = Context({'login_failed' : ugettext("This user is disabled."), 'user_name' : username, 'LANGUAGES':settings.LANGUAGES})
    else:
        t = loader.get_template('login_template.html')
        c = Context({'login_failed' : ugettext("User or Password invalid."), 'user_name' : username, 'LANGUAGES':settings.LANGUAGES})
    ...
    try:
        ...
        if user.check_password(old_password) :
            ...
            if new_password1 == new_password2 :
                ...
                data['error_message'] = ugettext('Confirm password is not correct.')
        else :
            data['error_message'] = ugettext('Your old password is not correct.')
    ...
    if user is not None:
        if user.is_active:
            ...
            if len(userCompany) == 1 and is_hr == 1:
                ...
                t = loader.get_template('login_template.html')
                c = Context({'login_failed' : "Unknown user type of this user", 'user_name' : username})
            # Redirect to a success page.
        else:
            # Return a 'disabled account' error message
            t = loader.get_template('login_template.html')
            c = Context({'login_failed' : "This user is deabled.", 'user_name' : username})
    else:
        t = loader.get_template('login_template.html')
        c = Context({'login_failed' : "User or Password invalid.", 'user_name' : username})
    ...
    try:
        ...
        if user.check_password(old_password) :
            ...
            if new_password1 == new_password2 :
                ...
                data['error_message'] = 'Confirm password is not correct.'
        else :
